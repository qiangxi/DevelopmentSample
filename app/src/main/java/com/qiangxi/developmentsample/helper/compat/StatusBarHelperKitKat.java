...
/**
 * 作者 任强强 on 2017/4/11 11:48.
 * [4.4-5.0) StatusBarHelper帮助类
 */
@TargetApi(Build.VERSION_CODES.KITKAT)
final class StatusBarHelperKitKat implements IStatusBar {
    ...
    public void setStatusBarColor(Activity activity, int statusColor) {
        ...
        if (view != null) {
            ...
            if (activity.getTheme().resolveAttribute(R.attr.actionBarSize, typedValue, true)) {
                ...
                StatusBarCompat.setContentTopPadding(activity, actionBarHeight);
            }
        }
    }
    ...
    @Override
    public void setStatusBarColorForCollapsingToolbar(final Activity activity, AppBarLayout appBarLayout,
    ...
    @Override
    public void setStatusBarLightForCollapsingToolbar(final Activity activity, AppBarLayout appBarLayout,
                                                      final CollapsingToolbarLayout collapsingToolbarLayout,
        ...
        appBarLayout.addOnOffsetChangedListener(new AppBarLayout.OnOffsetChangedListener() {
            ...
            @Override
            public void onOffsetChanged(AppBarLayout appBarLayout, int verticalOffset) {
                if (Math.abs(verticalOffset) >= (appBarLayout.getTotalScrollRange() - StatusBarCompat.getPxFromDp(activity, 56))) {
                    if (appBarLayoutState != COLLAPSED) {
                        appBarLayoutState = COLLAPSED;

                        if (StatusBarCompat.MIUISetStatusBarLightMode(activity, true) || StatusBarCompat.FlymeSetStatusBarLightMode(activity, true)) {
                        }
                        if (statusView.getAlpha() == 0) {
                            statusView.animate().cancel();
                            statusView.animate().alpha(1f).setDuration(collapsingToolbarLayout.getScrimAnimationDuration()).start();
                    ...
                    if (appBarLayoutState != EXPANDED) {
                        ...
                        if (StatusBarCompat.MIUISetStatusBarLightMode(activity, false) || StatusBarCompat.FlymeSetStatusBarLightMode(activity, false)) {
                        }
                        if (statusView.getAlpha() == 1) {
                            statusView.animate().cancel();
                            statusView.animate().alpha(0f).setDuration(collapsingToolbarLayout.getScrimAnimationDuration()).start();
